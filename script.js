    ...
    try {
        ...
        const fetchPromises = data.deals.map(async (deal) => {
            ...
            if (userDeal && userDeal.loot_box_asset_quantity_low && userDeal.loot_box_asset.price_usd) {
                profit = ((userDeal.loot_box_asset_quantity_low * userDeal.loot_box_asset.price_usd) - (2.68 * userDeal.cost_gold)).toFixed(2);
                ...
                profit = ((userDeal.asset_quantity * userDeal.asset.price_usd) - (2.68 * userDeal.cost_gold)).toFixed(2);
                document.getElementById(`first-p-${deal.id}`).textContent = 
